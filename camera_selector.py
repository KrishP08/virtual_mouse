...
class CameraSelector:
    ...
    def show_camera_selector(self, callback=None):
        ...
        self.selector_window.title("Select Camera")
        self.selector_window.geometry("600x400")
        ...
        # Title
        title_label = tk.Label(self.selector_window, text="Choose Camera", 
                             bg='#2b2b2b', fg='white', font=('Arial', 16, 'bold'))
        title_label.pack(pady=10)
        
        # Camera list frame
        list_frame = tk.Frame(self.selector_window, bg='#2b2b2b')
        list_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=10)
        
        # Create treeview for camera list
        ...
        # Preview frame
        preview_frame = tk.Frame(self.selector_window, bg='#2b2b2b')
        preview_frame.pack(fill=tk.X, padx=20, pady=10)
        
        # Preview button
        self.preview_btn = tk.Button(preview_frame, text="Preview Selected Camera", 
                                   command=self.toggle_preview, bg='#404040', fg='white',
                                   font=('Arial', 10))
        self.preview_btn.pack(side=tk.LEFT, padx=5)
        
        # Refresh button
        refresh_btn = tk.Button(preview_frame, text="Refresh Camera List", 
                               command=self.refresh_cameras, bg='#404040', fg='white',
                               font=('Arial', 10))
        refresh_btn.pack(side=tk.LEFT, padx=5)
        
        # Button frame
        button_frame = tk.Frame(self.selector_window, bg='#2b2b2b')
        button_frame.pack(pady=20)
        
        # Select button
        select_btn = tk.Button(button_frame, text="Select Camera", 
                             command=lambda: self.select_camera(callback),
                             bg='#006600', fg='white', font=('Arial', 12), width=15)
        select_btn.pack(side=tk.LEFT, padx=10)
        
        # Cancel button
        cancel_btn = tk.Button(button_frame, text="Cancel", 
                             command=self.cancel_selection,
                             bg='#660000', fg='white', font=('Arial', 12), width=15)
        ...
        self.selector_window.protocol("WM_DELETE_WINDOW", self.cancel_selection)
        
        # Center the window
        self.selector_window.update_idletasks()
        width = self.selector_window.winfo_width()
        height = self.selector_window.winfo_height()
        x = (self.selector_window.winfo_screenwidth() // 2) - (width // 2)
        y = (self.selector_window.winfo_screenheight() // 2) - (height // 2)
    ...
    def preview_loop(self):
        ...
        while self.preview_running:
            ...
            # Add preview text
            cv2.putText(frame, f"Camera {self.selected_camera} Preview", (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.putText(frame, "Press 'q' to close preview", (10, 60), 
...
def show_camera_selector():
    """Standalone function to show camera selector"""
    root = tk.Tk()
